...
Konzeptdokument · **Version 2** · Stand: 10. Juni 2026

> Änderungen gegenüber Version 1: Würfelanzahl (ein oder zwei) als offene Entscheidung markiert; Karten werden primär über eine digitale Begleit-App ausgespielt (vier Farben antippen, Karte öffnet sich, Audio wird vorgelesen), gedruckte Karten optional; Audio per Text-to-Speech oder echter Aufnahme; neuer Abschnitt zur technischen Architektur (GitHub-Repository, GitHub Pages, Supabase).
...
Alle Karten kommen aus einer strukturierten Datenquelle. In der Begleit-App liegt diese in **Supabase** (siehe Abschnitt 8); für den Test-Prototyp und spätere Druckvorlagen lässt sie sich als JSON exportieren. Vorschlag für die Felder pro Karte:
...
- **Code & Hosting:** Der Quellcode liegt in einem **GitHub-Repository** und wird über **GitHub Pages** als Website bereitgestellt. Der Link lässt sich auf dem Handy als Lesezeichen ablegen.
- **Datenbank:** **Supabase** ist die zentrale Datenquelle für die Karten. Die App liest die Karten je Bereich direkt über den Supabase-Client aus.
...
- **Hinweis:** GitHub Pages ist reines statisches Hosting; die Verbindung zur Datenbank läuft direkt vom Browser zu Supabase. Für die spätere Pflege der Inhalte genügt damit ein Bearbeiten der Daten in Supabase, ohne den Code anzufassen.

Der Test-Prototyp aus Schritt 2 bleibt davon getrennt: Er ist eine eigenständige Single-File-HTML-App zum Entwerfen und Austarieren des Bretts und braucht weder GitHub Pages noch Supabase.
...
Die gemeinsame Grundlage. Beschreibt Idee, Brett, Bereiche, Karten, Sprache, Audio, Technik und Zugänglichkeit.

### Schritt 2 – Digitaler Test-Prototyp mit Testumgebung
...
- **Einstellbar machen:** Anzahl Felder pro Bereich, Position der Kartenfelder, Anzahl Karten je Stapel, ein oder zwei Würfel.
- 2 bis 4 Spieler unterstützen.
...
- **Mehrere Spiele simulieren** und auswerten: durchschnittliche Spieldauer, Anzahl Züge, ob die Feld- und Kartenverteilung funktioniert – und ob sich die Würfelfrage damit beantworten lässt.
...
### Schritt 6 – Begleit-App entwickeln
